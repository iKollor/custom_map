...
import {
  getRenderableCoordinates,
  getSectionPolygonCoordinates,
  sortLinearFeatures,
} from "./map-client-utils"
...
function FeatureTooltip({
  ...
  return (
    <div
      ref={containerRef}
      className="pointer-events-auto max-w-64 min-w-44 space-y-2 text-left"
    >
...
function estimateEtaText(feature: ParsedFeature, distanceKm: number) {
  const etaRaw = findMetricValue(feature, [
    "eta_min",
    "eta_minutes",
    "eta",
    "tiempo_estimado",
...
function isPointCoords(
  coords: ParsedFeature["_coords"]
): coords is [number, number] {
  return Array.isArray(coords) && typeof coords[0] === "number"
}

function isLineCoords(
  coords: ParsedFeature["_coords"]
): coords is [number, number][] {
...
function LineFeatureTooltip({
  ...
  if (feature.type === "route") {
    const providerDistance = findMetricValue(feature, [
      "distance_km",
      "distance",
      "distancia_km",
      "distancia",
    ])
    const parsedDistance = providerDistance
      ? Number(providerDistance.replace(/[^\d.]/g, ""))
      : NaN
    const distanceKm =
    ...
    return (
      <div className="max-w-64 min-w-44 space-y-2 text-left">
        <div className="flex items-center gap-2">
          <span
            aria-hidden
            className="inline-block size-2 shrink-0 rounded-full ring-2 ring-background"
  ...
  return (
    <div className="max-w-72 min-w-48 space-y-2 text-left">
      <div className="flex items-center gap-2">
        <span
          aria-hidden
          className="inline-block size-2 shrink-0 rounded-full ring-2 ring-background"
      ...
      <div className="grid grid-cols-2 gap-2 rounded-md border border-border/60 bg-background/60 p-2">
        <div>
          ...
          <p className="text-xs font-semibold text-foreground">
            {lineDistanceKm(sectionPolygon).toFixed(2)} km
          </p>
        </div>
        <div>
          <p className="text-[9px] tracking-wide text-muted-foreground uppercase">
...
function RouteFeature({
  ...
  return (
    <>
      ...
      {tooltipsEnabled && !isOverMarker && hoverCoordinates && (
        <MapPopup
          ...
          closeOnClick={false}
          closeOnMove={false}
          wrapperClassName={`pointer-events-none section-tooltip-wrapper ${feature.type === "section" ? "z-0" : "z-10"}`}
...
export function MapFeatureLayers({
  ...
  const [isOverHtmlElement, setIsOverHtmlElement] = useState(false)
  const [isOverCanvasMarker, setIsOverCanvasMarker] = useState(false)
  const isOverMarker = isOverHtmlElement || isOverCanvasMarker
  const [armedDuplicatePointId, setArmedDuplicatePointId] = useState<
    string | null
  >(null)
  const [dragDuplicatePointId, setDragDuplicatePointId] = useState<
    string | null
  >(null)
  ...
  const duplicateGesturePointerTypeRef = useRef<
  ...
  useEffect(() => {
    const handleMouseMove = (e: MouseEvent) => {
      ...
      // If the target is not the canvas, we consider it over a marker or UI element.
      // We hide section tooltips in this case to prevent overlap.
      setIsOverHtmlElement(target?.tagName !== "CANVAS")
    }
  ...
  const handlePointPointerDown = (featureId: string, event: PointerEvent) => {
    ...
    duplicateGesturePointerTypeRef.current = event.pointerType as
      | "mouse"
      | "touch"
      | "pen"
  ...
  const pieOptions = useMemo(
    ...
    [clusterCategoryColors]
  )

  const coverageOverlayOptions = useMemo(
    () => ({
      enabled: true,
      fillOpacity: 0.14,
      outlineOpacity: 0.42,
  ...
  const handlePointClick = useCallback(
    (feature: GeoJsonFeature<GeoJsonPoint, any>) => {
      const properties = feature.properties
      if (!properties) return
      onSelectRouteAction(null)
      onOpenFeatureInfoAction(properties.id)
    },
    [onSelectRouteAction, onOpenFeatureInfoAction]
  )
  ...
  return (
    <>
      ...
      {sortedLinearFeatures.map((feature) => {
        ...
        const anchor =
          feature.type === "section"
            ? (computedCentroid ?? midpoint(coordinates))
      ...
      {/* Route endpoints */}
      {!editMode &&
        sortedLinearFeatures
          .filter((feature) => feature.type === "route")
          .flatMap((feature) => {
            const coordinates = getRenderableCoordinates(
              feature,
              resolvedRoutes
            )
            ...
            return [
              ...
              >
                <MarkerContent>
                  <div className="relative flex items-center justify-center">
                    <div
                      className={`absolute rounded-full bg-[#40a7f4]/24 ${selected ? "size-8 animate-ping" : "size-6"}`}
                    />
              ...
              >
                <MarkerContent>
                  <div className="relative flex items-center justify-center">
                    <div
                      className={`absolute rounded-full bg-[#6e00a3]/24 ${selected ? "size-8 animate-ping" : "size-6"}`}
                    />
      ...
      {editMode || !clusteringEnabled
        ? pointFeatures.map((feature) => {
            ...
            return (
                ...
                onDragEnd={({ lng: nextLng, lat: nextLat }) => {
                  ...
                  const pixelDistance =
                    origin && current
                      ...
                      : 0
                  const movedInCoords =
                    Math.hypot(nextLng - lng, nextLat - lat) >
                    DUPLICATE_COORD_EPSILON
                  ...
                  if (dragDuplicatePointId === feature._id) {
                    ...
                    const duplicatedId = onDuplicatePointFeatureAction(
                      feature._id,
                      nextPoint
                    )
                    if (duplicatedId) {
                  ...
                  endGestureTracking()
                  onUpdateFeatureCoordinatesAction(feature._id, [
                    nextLng,
                    nextLat,
                  ])
                  onOpenFeatureInfoAction(feature._id)
              ...
              >
                <MarkerContent className="animate-in duration-300 fade-in-0 zoom-in-75">
                  <div
                  ...
                  >
                    ...
                    {duplicateReady && (
                      <div className="pointer-events-none absolute -top-8 rounded-full border border-sky-200/70 bg-sky-500/90 px-2 py-0.5 text-[9px] font-semibold tracking-wide whitespace-nowrap text-white uppercase shadow-lg shadow-sky-700/30">
        ...
        : clusterData.features.length > 0 && (
            <MapClusterLayer
              ...
              renderPointTooltip={isMobile ? undefined : renderPointTooltip}
              pointTooltipClassName="bg-popover text-popover-foreground rounded-md border px-2.5 py-1.5 shadow-md"
              pieOptions={pieOptions}
              coverageOverlayOptions={coverageOverlayOptions}
              onPointClick={handlePointClick}
              onPointHoverChange={setIsOverCanvasMarker}
            />
          )}
      ...
      {editMode &&
        sortedLinearFeatures.flatMap((feature) => {
          ...
          if (
            ...
            !Array.isArray(sourceCoordinates[0])
          )
            return []
          ...
          return lineCoordinates.map((point, index) => {
            ...
            const isEndpoint =
              feature.type === "route"
                ? index === 0 || index === lineCoordinates.length - 1
                : false
            const isSelected = activeSelectedRouteId === feature._id
              ...
              <MapMarker
                ...
                onDragEnd={({ lng: nextLng, lat: nextLat }) => {
                  ...
                  const isClosed =
                    ...
                    lineCoordinates.length >= 4 &&
                    lineCoordinates[0]?.[0] ===
                      lineCoordinates[lineCoordinates.length - 1]?.[0] &&
                    lineCoordinates[0]?.[1] ===
                      lineCoordinates[lineCoordinates.length - 1]?.[1]

                  const nextCoordinates = lineCoordinates.map(
                    (coord, coordIndex) => {
                      if (isClosed) {
                        if (
                          index === 0 &&
                          coordIndex === lineCoordinates.length - 1
                        )
                          return [nextLng, nextLat] as [number, number]
                        if (
                          index === lineCoordinates.length - 1 &&
                          coordIndex === 0
                        )
      ...
      {/* Forced Tooltips */}
      {[...pointFeatures, ...linearFeatures]
        .filter(
          (f) =>
            forcedTooltipTypes.has(f.type) ||
            forcedTooltipCategories.has(f.categoryId)
        )
        .map((feature) => {
            ...
            >
              {feature.type === "point" ? (
                ...
                <LineFeatureTooltip
                  feature={feature}
                  categories={categories}
                  coordinates={getRenderableCoordinates(
                    feature,
                    resolvedRoutes
                  )}
                  sectionPolygon={
